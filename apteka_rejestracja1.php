...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
      ...
      <div class="form-group row">
        <label for="inputPassword3" class="col-sm-8 col-form-label">Nazwa apteczki</label>
        <div class="col-sm-12">
          <input type="text" class="form-control" id="inputApteczka" name="apteczka" placeholder="Nazwa apteczki">
        </div>
      </div>
...
$user_fullname=$user_email=$user_password=$apteczka="";
    ...
    return $dane;
        ...
        if ($_SERVER["REQUEST_METHOD"]=="POST") {
            ...
            $user_password_hash=password_hash($user_password,PASSWORD_DEFAULT);
                ...
                if((!empty($user_email))and(!empty($user_password))and(!empty($user_fullname))and(!empty($apteczka))){
                    $query3="SELECT * FROM `users` WHERE `user_email`='$user_email'";
                    $result3 = mysqli_query($dbconn, $query3);
                    $ilosc3=mysqli_num_rows($result3);
                    
                    if ($ilosc3==0) {
                    $result2=mysqli_query($dbconn, "INSERT INTO apteczki ( `Id_apteczki`, `Nazwa_apteczki`)
                    VALUES (NULL, '".$apteczka."')");
                    $searchid = "SELECT Id_apteczki FROM apteczki WHERE Nazwa_apteczki='$apteczka'";
                    $myresult=mysqli_query($dbconn,$searchid);
                    ...
                    if($rzedy==1) {
                        ...
                        $result=mysqli_query($dbconn, "INSERT INTO users ( `user_email`, `user_passwordhash`, `user_fullname`, `apteczka_ID`)
                        VALUES ( '".$user_email."', '".$user_password_hash."','".$user_fullname."', '".$id_apteczki."')");
                        if (!$result)
                            {
                    ...
                    <p class="alert-heading ">Apteczka o takiej nazwie już istnieje! </p>
                    </div>';
