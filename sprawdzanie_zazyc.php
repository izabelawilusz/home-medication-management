  ...
  <div class="container-fluid h-custom">
    <div class="row d-flex justify-content-center align-items-center h-100">
      <div class="col-md-9 col-lg-6 col-xl-5">
          ...
          <p  class="text-center h4 fw-bold mb-5 mx-1 mx-md-4 mt-4">Sprawdź kto i kiedy zażył</p>
</div>
...
<div class="form-group d-flex flex-row align-items-center mb-4">
<div class=" col-5">
    <label for="uzytkownicy">Użytkownik</label>
</div>
<div class=" col-5">
      ...
      <?php
        ...
        $rzedy=mysqli_num_rows($result1);
            ...
            $result2 = "SELECT user_fullname FROM users WHERE `apteczka_ID`='$id_apt'";
            echo $result;
            
            $runquery = mysqli_query($conn,$result2);

            while($row = mysqli_fetch_assoc($runquery)){
            $val = $row['user_fullname'];
...
$query = "SELECT users.user_fullname, historia_operacji.typ_operacji, dostepne_leki.Nazwa_leku, historia_operacji.ilosc, historia_operacji.user_id,  historia_operacji.data_operacji, users.user_fullname FROM historia_operacji INNER JOIN dostepne_leki ON dostepne_leki.ID_leku=historia_operacji.lek_id INNER JOIN users ON users.user_id=historia_operacji.user_id WHERE historia_operacji.apteczka_id='$id_apt'  AND `data_operacji` >= '$date_from' AND `data_operacji` <= '$date_to' AND `user_fullname`='$uzytkownicy' ORDER BY `historia_operacji`.`data_operacji` DESC";

$result = mysqli_query($conn,$query);

echo '<p class="h5 py-3"> Użytkownik '.$uzytkownicy.' w okresie od: '.$date_from.' do '.$date_to.' zażył: </p>' ;

?>

<table class="table table-bordered table-sm text-center" style="background-color: white">
    <thead>
    <tr style="background-color: #C4BDB8"><th>Użytkownik</th><th>Nazwa leku</th><th>Operacja</th><th>Ilość</th><th>Data operacji</th></tr>
...
if (mysqli_num_rows($result)>0) {
    while($row=mysqli_fetch_assoc($result)){
        echo "<tr><td>".$row["user_fullname"]."</td><td>".$row["typ_operacji"]."</td><td>".$row["Nazwa_leku"]."</td><td>".$row["ilosc"]."</td><td>".$row["data_operacji"]."</td></tr>";
...
<table class="table table-bordered table-sm text-center" style="background-color: white">
    <thead>
    <tr style="background-color: #C4BDB8"><th>Użytkownik</th><th>Operacja</th><th>Nazwa leku</th><th>Ilość</th><th>Cena [1 szt.]</th><th>Data operacji</th></tr>
...
if (mysqli_num_rows($result)>0) {
    while($row=mysqli_fetch_assoc($result)){
        echo "<tr><td>".$row["user_fullname"]."</td><td>".$row["typ_operacji"]."</td><td>".$row["Nazwa_leku"]."</td><td>".$row["ilosc"]."</td><td>".$row["cena_leku"]."</td><td>".$row["data_operacji"]."</td></tr>";
